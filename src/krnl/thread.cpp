...
//    thrd->stack_end = stack - stack_size;
//
...
os_pcb *register_thread_auto(volatile void (*thread_handler)(), uint32_t stack_size, uint32_t *stack, os_pcb * thrd, uint8_t priority, char* name) {
    thrd->stack_begin = stack;
    thrd->rdy = 1;
    thrd->priority = priority;
    thrd->name = name;
