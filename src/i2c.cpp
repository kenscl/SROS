...
void configure_i2c() {
    ...
    GPIOB->CRH &= ~(0xf << 12); // clear CNF10
    GPIOB->CRH |= (0xb << 8); // Mode 10 to output mode max 2MHz
    GPIOB->CRH |= (0xb << 12); // CNF10 to Alternate function open drain 
...
void start_i2c_communication() {
    ...
    while (!(I2C2->SR1 & I2C_SR1_SB)); // wait for start condition generated
}
