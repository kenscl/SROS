...
void SPI_send_next() {
  if (queue[queue_head]->size != 0) {
    if (current->state == Done) {
      ...
      current->target->send_data(current->adress, current->rx_buffer,
...
// SPI
uint8_t SPI_handle(SPI_information *info) { return enqueue(info); }
...
static SPI_target dmy_target;

static SPI_information dmy = {.state = Done,
                              .size = 0,
                              .rx_buffer = nullptr,
                              .tx_buffer = nullptr,
                              .target = &dmy_target,
