...
void LSM9DS1_process_accel() {
    ...
    LSM9DS1_acc_filtered = LSM9DS1_acc_filtered.normalize();
}
...
void LSM9DS1_process_mag() {
    ...
    LSM9DS1_mag_filtered = LSM9DS1_mag_filtered.normalize();
...
void low_pass_filter(float alpha, Vec3 *mean, Vec3 *new_measurement) {
    *mean = *mean * alpha + *new_measurement * (1 - alpha);
}
...
volatile void LSM9DS1_thread() {
    ...
    while (1) {
        ...
        if (DEBUG == 2 && !spi_busy) {
            ...
            LSM9DS1_gyro_filtered.print_bare();

            os_printf("[LSM9DS1_acc] ");
            LSM9DS1_acc_filtered.print_bare();

            os_printf("[LSM9DS1_mag] ");
            LSM9DS1_mag_filtered.print_bare();
            scheduler_enable();
        }
