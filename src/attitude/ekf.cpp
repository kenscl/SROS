...
void EKF::init(Vec3 *gyro, Vec3 *acc, Vec3 *mag) {
  ...
  this->update_mag(mean_mag, mean_acc);
  ...
  float mag_x_comp = mean_mag[0] * cos(pitch_init) + mean_mag[1] * sin(roll_init) * sin(pitch_init) + mean_mag[2] * sin(pitch_init) * cos(roll_init);
  float mag_y_comp = mean_mag[1] * cos(roll_init) - mean_mag[2] * sin(roll_init);

  float yaw_init = atan2(mag_y_comp, -mag_x_comp);
  if (yaw_init > M_PI)
    yaw_init -= 2.0 * M_PI;
  ...
  float s_gyro = 
      (sqrt(gyro_sum[0] / (num_init - 1)) + sqrt(gyro_sum[1] / (num_init - 1)) +
       sqrt(gyro_sum[2] / (num_init - 1))) /
      3;
  float s_acc =
      (sqrt(acc_sum[0] / (num_init - 1)) + sqrt(acc_sum[1] / (num_init - 1)) +
       sqrt(acc_sum[2] / (num_init - 1))) /
      3;
  float s_yaw = 1 * sqrt(mag_sum / (num_init - 1));
...
void EKF::update_mag(Vec3 mag, Vec3 acc) {
  ...
  Vec3 heading = this->attitude.to_rpy();
  float roll = heading[0];
  float pitch = heading[1];
  float mag_x_comp = mag[0] * cos(pitch) + mag[1] * sin(roll) * sin(pitch) + mag[2] * sin(pitch) * cos(roll);
  float mag_y_comp = mag[1] * cos(roll) - mag[2] * sin(roll);
...
void EKF::predict(Vec3 gyro, float dt) {
  ...
  Vec3 z_acc;
  z_acc[0] = 2 * (q.i * q.k - q.q * q.j);
  z_acc[1] = 2 * (q.j * q.k + q.q * q.i);
  z_acc[2] = 1 - 2 * (q.i * q.i + q.j * q.j);
...
void EKF::update() {
  ...
  //              (Mat<10, 10>().identity() - (this->K * this->H)).transpose() +
...
volatile void attitude_thread() {
  ...
  while (1) {
    ...
    if (has_init) {
      ...
      // mag update
      if (next_mag_time < now()) { // we only get this every n milliseconds, so setting the value more often is 
        next_mag_time = now() + 50 * MILLISECONDS;
        ekf.update_mag(LSM9DS1_mag_filtered.normalize(), LSM9DS1_acc_filtered.normalize());
	// acc update
	ekf.update_acc(LSM9DS1_acc_filtered.normalize());
      ...
      // state prediction
      ekf.predict(LSM9DS1_gyro_filtered / 180 * M_PI, dt);
    ...
    // init
    if (acc_cnt < 20) {
      acc[acc_cnt] = LSM9DS1_acc.normalize();
      acc_cnt++;
    }
    ...
    if (next_mag_time < now() && mag_cnt < 20) {
      next_mag_time = now() + 15 * MILLISECONDS;
      mag[mag_cnt] = LSM9DS1_mag.normalize();
      mag_cnt++;
    }
